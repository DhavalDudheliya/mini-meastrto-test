...
const SubmissionDialog = ({ isOpen, onClose, order_id, product_id }: SubmissionDialogProps) => {
  ...
  const handleSubmit = () => {
    ...
    if (message === "" || !message) {
      setTextAreaError(t("message_cant_empty"));
      return;
    }
                ...
                <div className="flex flex-col gap-2 xl:gap-4">
                  ...
                  <div>
                    <div className="flex flex-col gap-1">
                      ...
                      <textarea 
                        onChange={(e) => {setMessage(e.target.value); setTextAreaError('');}} 
                        id="message" 
                        className={`h-[135px] resize-none ${textAreaError ? 'border-red-500' : ''}`} 
                      />
                        <span className="text-red-500">{textAreaError}</span>
                    </div>
                  </div>
                </div>
              </div>
